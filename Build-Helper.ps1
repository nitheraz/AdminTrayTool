...
while ($true) {
	...
	Write-Host "[5] Commit, Merge to Main, Push & Trigger GitHub Release"
	Write-Host "[6] Exit`n"
	...
	switch ($choice) {
		"1" {
			...
			if ((Test-Path "$WiXPath\heat.exe") -and
		...
		"5" {
			...
			if ($pushResp -eq "y") {
				...
				Write-Host "`nPush cancelled. No GitHub release will be created.`n" -ForegroundColor Yellow
				break
			}
